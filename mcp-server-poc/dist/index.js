...
class PostgresMCPServer {
    ...
    async executeQuery(args) {
        ...
        if (!validation.success) {
            throw new McpError(ErrorCode.InvalidParams, `Invalid parameters: ${validation.error.message}`);
        }
    ...
    async listTables(args) {
        ...
        try {
            client = await this.pool.connect();
            const result = await client.query(`SELECT table_name, table_type 
    ...
    async describeTable(args) {
        ...
        try {
         ...
         ORDER BY ordinal_position`, [schema, table]);
            ...
            const constraintsResult = await client.query(`SELECT 
           tc.constraint_name,
           tc.constraint_type,
           kcu.column_name
    ...
    async getTableData(args) {
        ...
        try {
            ...
            const result = await client.query(`SELECT * FROM ${schemaName}.${table} LIMIT $1`, [limit]);
            return {
                content: [
                    {
                        type: "text",
